...
func _post_import(scene):
	var traverse_base = scene.get_child(0).get_child(0).get_child(0)
	var l = []
	for t1 in traverse_base.get_children():
		for t2 in t1.get_children():
			#push_warning("{{%s}}" % scene.get_path_to(t2))
			pass
	return scene
